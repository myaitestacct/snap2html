...
- `template.html` — the Snap2HTML 2.5 output template (reference for the
  current file format).
- `shows/consolidate_shows.ps1` — flatten `shows-A_2_R.html` and
...
## consolidate_shows.ps1

`shows/consolidate_shows.ps1` builds a **single-root** V2 snapshot from
`shows-A_2_R.html` and `shows-S_2_Z.html` (or any other V2 snapshots you
pass it) by filling `template.html`:

```powershell
.\shows\consolidate_shows.ps1

# equivalent, with explicit paths
.\shows\consolidate_shows.ps1 shows\shows-A_2_R.html shows\shows-S_2_Z.html `
...
- The general merger keeps each input's root folder, so two snapshots of
  ...
  trees.
- `consolidate_shows.ps1` **drops** those original roots and lists every
